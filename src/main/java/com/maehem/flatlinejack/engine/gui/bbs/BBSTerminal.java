 ...
 * @author mark
 */
public class BBSTerminal extends TextFlow {
    private static final double LINE_SPACE = 0.0; // default is 0.0
    private static final Color BG_COLOR = new Color(0.2, 0.2, 0.2, 1.0);
    private static final double FONT_H = 32;

    private final BBSText header; //= new BBSText();
    private final BBSText body;   //= new BBSText();
    private final BBSText footer; //= new BBSText();
    ...
    public BBSTerminal(GameState gs, int width, int height, int rows, int cols) {
        ...
        //Text header = new AbyssHeader();
        setHeader(SiteHeader.CYBERIA);
        setBody("1\n2\n3\n4\n5");
