...
import com.maehem.abyss.engine.matrix.MatrixNodeFactory;
...
import com.maehem.abyss.engine.matrix.SoftwareTabNode;
 ...
 * @author mark
 */
public class MatrixPane extends ViewPane implements GameStateListener {
...
//        root.getChildren().add(testBox);
    ...
    private void updateSiteNodes() {
        LOGGER.log(Level.INFO, "Rebuild Site Nodes.");
    ...
    public void updateHudSoftwareTabs() {
        ...
        if (currentDeck != null) {
            ...
            for (int i = 0; i < loadout.length; i++) {
                ...
                hudTabs.getChildren().add(new SoftwareTabNode(gameState,st, (char) ('0' + i + 1)));                    
            }
        } else { // Empty hudTabs
            LOGGER.log(Level.INFO, "    No current deck: Fill blank HUD.");
            for (int i = 0; i < 4; i++) {
                hudTabs.getChildren().add(new SoftwareTabNode(gameState, new EmptySoftwareThing(), (char)('0' + i + 1)));
            }
        }
    ...
    private ImageView backgroundImage() {
        Image im = new Image(getClass().getResourceAsStream("/background-1.png"));
    ...
    public void gameStateMatrixSiteChanged(GameState gs, int newAddr) {
        ...
        if ( scooted ) {
            scooted = false; // We moved, causing this event, but we don't rebuild the view.
            return;
        }
